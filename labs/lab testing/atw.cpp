...
//            client.println("<link rel='stylesheet' href='//netdna.bootstrapcdn.com/bootstrap/3.1.1/css/bootstrap.min.css'><script type='text/javascript' src='http://code.jquery.com/jquery-2.1.1.min.js'></script><script type='text/javascript'>$.ajax({url: 'http://107.170.57.28/return.php', type: 'get', success: function (response){$('body').html(response)}});</script></head><body></body>");
            client.println("<link rel='stylesheet' href='//netdna.bootstrapcdn.com/bootstrap/3.1.1/css/bootstrap.min.css'><script type='text/javascript' src='http://code.jquery.com/jquery-2.1.1.min.js'></script><script type='text/javascript'>$.ajax({url: 'http://10.63.43.201:8888/site/return.php', type: 'get', success: function (response){$('body').html(response)}});</script></head><body></body>");
            client.println("</head>");
            client.println("</html>");
          }else{
            client.println();
            httpResp = "{\"sensorLog\": \"" + (String)(sensorLog) + "\",\"action\": \"" + action + "\"}";
            Serial.println(httpResp);
            client.println(httpResp);
            httpResp = "";
          }
          break;
        }
...
void arm(){
  digitalWrite(3, LOW);
  sensorLog += ("calibrating sensor <br>");
  sensorLog += (" done <br>");
  sensorLog += (" SENSOR ACTIVE <br>");
}
...
void sense(){
  if(digitalRead(pirPin) == HIGH){
    ...
    if(lockLow){  
      ...
      lockLow = false;
     // sensorLog += "<br />motion detected at "+(String)(millis()/1000)+" sec";
      Serial.println(sensorLog);
    }         
    takeLowTime = true;
  }
  ...
  if(digitalRead(pirPin) == LOW){       
    ...
    if(!lockLow && millis() - lowIn > pause){  
        ...
        lockLow = true;                        
       // sensorLog += "<br />motion ended at "+(String)((millis() - pause)/1000)+" sec";
        Serial.println(sensorLog);
    }
  }
}
